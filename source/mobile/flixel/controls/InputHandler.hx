...
class InputHandler extends FlxSpriteGroup {
    ...
    public function checkOverlap(rect:FlxSprite):Bool {
        ...
        for (touch in FlxG.touches.list) {
            ...
            for (dz in deadzones) {
                if (dz != null && touch.overlaps(dz, camera)) {
                    inDeadzone = true;
                    break;
                }
            }

            if (!inDeadzone && touch.overlaps(rect, camera)) {
                overlap = true;
        ...
        if (FlxG.mouse.pressed) {
            var inDeadzone = false;
            for (dz in deadzones) {
                if (dz != null && FlxG.mouse.overlaps(dz, camera)) {
                    inDeadzone = true;
                    break;
                }
            }

            if (!inDeadzone && FlxG.mouse.overlaps(rect, camera)) {
                overlap = true;
                currentPointerID = -2;
            }
        }
